...
public class FFmpegFrameGrabber extends FrameGrabber {
	public static String[] getDeviceDescriptions() throws Exception {
		...
		throw new UnsupportedOperationException(
				"Device enumeration not support by FFmpeg.");
	}
	...
	public static FFmpegFrameGrabber createDefault(int deviceNumber)
			throws Exception {
		throw new Exception(FFmpegFrameGrabber.class
				+ " does not support device numbers.");
	}
	...
	public int getPixelFormat() {
		if (imageMode == ImageMode.COLOR || imageMode == ImageMode.GRAY) {
			if (pixelFormat == AV_PIX_FMT_NONE) {
				return imageMode == ImageMode.COLOR ? AV_PIX_FMT_BGR24
						: AV_PIX_FMT_GRAY8;
			} else {
				return pixelFormat;
			}
	...
	public String getMetadata(String key) {
		...
		return entry == null || entry.value() == null ? null : entry.value()
				.getString();
	}
	...
	public void setTimestamp(long timestamp) throws Exception {
		...
		if (oc == null) {
			...
			if ((ret = avformat_seek_file(oc, -1, Long.MIN_VALUE, timestamp,
					Long.MAX_VALUE, AVSEEK_FLAG_BACKWARD)) < 0) {
				throw new Exception("avformat_seek_file() error " + ret
						+ ": Could not seek file to timestamp " + timestamp
						+ ".");
			}
			if (video_c != null) {
			 ...
			 * av_frame_get_best_effort_timestamp in grabFrame sets
			 * this.timestamp to ...666 and the given timestamp has been rounded
			 * to ...667 (or vice versa)
			 */
	...
	public void startUnsafe() throws Exception {
		...
		if (format != null && format.length() > 0) {
			if ((f = av_find_input_format(format)) == null) {
				throw new Exception(
						"av_find_input_format() error: Could not find input format \""
								+ format + "\".");
			}
		}
		...
		if (pixelFormat >= 0) {
			av_dict_set(options, "pixel_format",
					av_get_pix_fmt_name(pixelFormat).getString(), 0);
		} else if (imageMode != ImageMode.RAW) {
			av_dict_set(options, "pixel_format",
					imageMode == ImageMode.COLOR ? "bgr24" : "gray8", 0);
		}
		if (imageWidth > 0 && imageHeight > 0) {
			av_dict_set(options, "video_size", imageWidth + "x" + imageHeight,
					0);
		}
		if (sampleRate > 0) {
		...
		if ((ret = avformat_open_input(oc, filename, f, options)) < 0) {
			...
			if ((ret = avformat_open_input(oc, filename, f, options)) < 0) {
				throw new Exception("avformat_open_input() error " + ret
		...
		if ((ret = avformat_find_stream_info(oc,
				(PointerPointer<AVDictionary>) null)) < 0) {
			throw new Exception("avformat_find_stream_info() error " + ret
		...
		// Find the first video and audio stream, unless the user specified
		// otherwise
		...
		if (video_st == null && audio_st == null) {
			throw new Exception(
					"Did not find a video or audio stream inside \"" + filename
							+ "\" for videoStream == " + videoStream
							+ " and audioStream == " + audioStream + ".");
		}
		...
		if (video_st != null) {
			...
			// some codecs
			if (video_c.time_base().num() > 1000
					&& video_c.time_base().den() == 1) {
			...
			if ((picture = av_frame_alloc()) == null) {
				throw new Exception(
						"av_frame_alloc() error: Could not allocate raw picture frame.");
			}
			if ((picture_rgb = av_frame_alloc()) == null) {
				throw new Exception(
						"av_frame_alloc() error: Could not allocate RGB picture frame.");
			}

			int width = getImageWidth() > 0 ? getImageWidth() : video_c.width();
			int height = getImageHeight() > 0 ? getImageHeight() : video_c
					.height();
			...
			case GRAY:
				...
				image_ptr = new BytePointer[] { new BytePointer(av_malloc(size))
						.capacity(size) };
				image_buf = new Buffer[] { image_ptr[0].asBuffer() };

				// Assign appropriate parts of buffer to image planes in
				// picture_rgb
				// Note that picture_rgb is an AVFrame, but AVFrame is a
				// superset of AVPicture
				avpicture_fill(new AVPicture(picture_rgb), image_ptr[0], fmt,
						width, height);
		...
		if (audio_st != null) {
			...
			if ((samples_frame = av_frame_alloc()) == null) {
				throw new Exception(
						"av_frame_alloc() error: Could not allocate audio frame.");
			}
		}
	}
	...
	public void trigger() throws Exception {
		if (oc == null || oc.isNull()) {
			throw new Exception(
					"Could not trigger: No AVFormatContext. (Has start() been called?)");
		}
		if (pkt2.size() > 0) {
			pkt2.size(0);
			av_free_packet(pkt);
	...
	private void processImage() throws Exception {
		...
		case GRAY:
			...
			if (deinterlace) {
				...
				avpicture_deinterlace(p, p, video_c.pix_fmt(), video_c.width(),
						video_c.height());
			}
			...
			img_convert_ctx = sws_getCachedContext(img_convert_ctx,
					...
					getImageWidth(), getImageHeight(), getPixelFormat(),
			...
			sws_scale(img_convert_ctx, new PointerPointer(picture),
					picture.linesize(), 0, video_c.height(),
					new PointerPointer(picture_rgb), picture_rgb.linesize());
			frame.imageStride = picture_rgb.linesize(0);
			// System.out.println("frame.imageStride " + frame.imageStride +
			// " image width " + frame.imageWidth);
		...
		case RAW:
			...
			if (ptr != null && !ptr.equals(image_ptr[0])) {
				image_ptr[0] = ptr.capacity(frame.imageHeight
						* frame.imageStride);
	...
	public Frame grab() throws Exception {
		// change no need to process audio.
	...
	public Frame grabFrame(boolean processImage, boolean doAudio,
			boolean keyFrames) throws Exception {
		if (oc == null || oc.isNull()) {
			throw new Exception(
					"Could not grab: No AVFormatContext. (Has start() been called?)");
		}
		...
		while (!done) {
			...
			if (video_st != null && pkt.stream_index() == video_st.index()
				...
				// Decode video frame [out] picture, [in] pkt, [in,out]
				// got_frame
				int len = avcodec_decode_video2(video_c, picture, got_frame,
						pkt);
				...
				if (len >= 0
					...
					timestamp = 1000000L * pts * time_base.num()
							/ time_base.den();
					// best guess, AVCodecContext.frame_number = number of
					// decoded frames...
				...
				// Decode audio frame
				int len = avcodec_decode_audio4(audio_c, samples_frame,
						got_frame, pkt2);
				if (len <= 0) {
					...
					if (got_frame[0] != 0) {
						...
						int planes = av_sample_fmt_is_planar(sample_format) != 0 ? (int) samples_frame
								.channels() : 1;
						int data_size = av_samples_get_buffer_size(
								(IntPointer) null, audio_c.channels(),
						...
						for (int i = 0; i < planes; i++) {
							...
							if (!p.equals(samples_ptr[i])
									|| samples_ptr[i].capacity() < data_size) {
