...
public class CanvasFrame extends JFrame {
	...
	public static double getGamma(GraphicsDevice screen) {
		ColorSpace cs = screen.getDefaultConfiguration().getColorModel()
				.getColorSpace();
		if (cs.isCS_sRGB()) {
			return 2.2;
		} else {
			try {
				return ((ICC_ProfileRGB) ((ICC_ColorSpace) cs).getProfile())
						.getGamma(0);
	...
	public static GraphicsDevice getScreenDevice(int screenNumber)
		...
		if (screenNumber >= screens.length) {
			throw new Exception("CanvasFrame Error: Screen number "
					+ screenNumber + " not found. " + "There are only "
	...
	private void init(final boolean fullScreen, final DisplayMode displayMode,
			final double gamma) {
		Runnable r = new Runnable() {
			public void run() {
				...
				if (displayMode != null && d != null) {
					...
					d2 = new DisplayMode(w > 0 ? w : d.getWidth(), h > 0 ? h
							: d.getHeight(), b > 0 ? b : d.getBitDepth(),
	...
	protected void initCanvas(boolean fullScreen, DisplayMode displayMode,
			double gamma) {
		canvas = new Canvas() {
			@Override
				...
				try {
					...
					do {
						do {
							...
							if (buffer != null) {
								g.drawImage(buffer, 0, 0, getWidth(),
										getHeight(), null);
							}
							g.dispose();
	...
	// Latency is about 60 ms on Metacity and Windows XP, and 90 ms on Compiz
	// Fusion,
	// but we set the default to twice as much to take into account the
	// roundtrip
	...
	public void setCanvasSize(final int width, final int height) {
		...
		Runnable r = new Runnable() {
			public void run() {
				// There is apparently a bug in Java code for Linux, and what
				// happens goes like this:
				// 1. Canvas gets resized, checks the visible area (has not
				// changed) and updates
				// BufferStrategy with the same size. 2. pack() resizes the
				// frame and changes
				// the visible area 3. We call Canvas.setSize() with different
				// dimensions, to make
				// it check the visible area and reallocate the BufferStrategy
				// almost correctly
	...
	public Graphics2D createGraphics() {
		if (buffer == null || buffer.getWidth() != canvas.getWidth()
				|| buffer.getHeight() != canvas.getHeight()) {
			BufferedImage newbuffer = canvas.getGraphicsConfiguration()
					.createCompatibleImage(canvas.getWidth(),
							canvas.getHeight(), Transparency.TRANSLUCENT);
			if (buffer != null) {
	...
	public void showImage(Frame image, boolean flipChannels) {
		showImage(converter
				.getBufferedImage(
						image,
						converter.getBufferedImageType(image) == BufferedImage.TYPE_CUSTOM ? 1.0
								: inverseGamma, flipChannels, null));
	}
		...
		for (int i = 0; i < frames.length; i++) {
			...
			try {
				...
				while (!movedListener.moved && count < 5) {
					// wait until the window manager actually places our
					// window...
			...
			canvasMaxY = Math.max(canvasMaxY,
					frames[i].getY() + frames[i].getHeight());
			if ((i + 1) % canvasCols == 0) {
				canvasX = 0;
				canvasY = canvasMaxY;
			}
